...
void imprimir_tabla(list * procesos,int nqueues,int tiempo_total,sequence_item*, int);
void tiempo_espera_all(list* procesos, int tiempo,process* proceso_actual);
sequence_item * create_sequences(int);

void print_graph();
void write_graph(list * procesos,char* nombre ,int,int);
void init_graph(list* procesos , int range_x , int range_y);
void escribir(int pos,int t_ini, int t_fin,int flecha, int color);
int index_proceso(list* procesos,char* name);
process* retornar_proceso(list* procesos,int index);
FILE * fp ;
int NUM_SEQ = 50;
int arrow= 0;
/* Programa principal*/
...
void schedule(list * processes, priority_queue *queues, int nqueues) {
         ...
         sequence_item *sequencia =create_sequences(NUM_SEQ);
   ...
   imprimir_tabla(processes,nqueues,tiempo_actual,sequencia,tiempo_inicial);
}
void tiempo_espera_all(list* procesos, int tiempo,process *proceso_actual){
   node_iterator it;
   process *p;
...
void imprimir_tabla(list * procesos, int nqueues,int tiempo_total,sequence_item* sequencia,int tiempo_inicial){
   ...
   char * name ;
   int t_exec ;
   init_graph(procesos,tiempo_total,procesos->count);
   printf("------------------------------------------------------------\n");
   ...
   for(int i=  0; i < NUM_SEQ;i++){
      name = sequencia[i].name;
      t_exec = tiempo_inicial + sequencia[i].time;
      printf("[%s (%d)]%s",sequencia[i].name, sequencia[i].time,(sequencia[i+1].name != 0)? "->": "");
      //printf("%s",(i%3==0)?"\n":"");
      write_graph(procesos,name,tiempo_inicial,t_exec);
      //printf("%s , %d t ini %d \n",name,t_exec,tiempo_inicial);
      tiempo_inicial = tiempo_inicial + sequencia[i].time;
      if(sequencia[i+1].name == 0){
         break;
      }
   }
   print_graph();
...
void write_graph(list* procesos,char* name,int t_inicio,int t_fin){
   node_iterator it;
   process *p;
   
   //int i =flecha;
   int pos=0;
   pos = index_proceso(procesos,name);
   
   arrow = arrow +1;
   escribir(pos,t_inicio,t_fin,arrow,1);
      
   for(int k = 1 ; k<=procesos->count;k++){
      arrow = arrow +1;
      if(k!=pos ){
         p = retornar_proceso(procesos,k);
         if(p->finished_time>t_inicio ){
            escribir(k,t_inicio,t_fin,arrow,2);
         } 
         if(p->arrival_time>t_inicio){
            escribir(k,p->arrival_time,t_fin,arrow,2);
         }
      }
   }
...
void escribir(int pos,int t_ini, int t_fin,int flecha, int color){
   ...
   fprintf(fp, "set arrow %d from %d,%d to %d,%d as %d\n",flecha , t_ini , pos ,t_fin, pos, color);
}
void print_graph(){
   fprintf(fp, "plot NaN");
   fclose(fp);
   //Generar el grafico PNG
   system("gnuplot plot_2.plt");

   printf("\n\nGrafica generada en plot_final.png\n");
}
void init_graph(list* procesos , int range_x , int range_y){
   fp = fopen("plot_2.plt", "w");
   node_iterator it;
   process *p;
   int i = 1;
    if (fp == NULL) {
   ...
   fprintf(fp, "set output '../plot_final.png'\n");
   fprintf(fp, "set style fill solid\n");
   fprintf(fp, "set xrange [0:%d]\n",range_x+5);
   fprintf(fp, "set yrange [0:%d ]\n",range_y+1);
   fprintf(fp, "unset ytics\n");
   fprintf(fp, "set title 'Planificacion ppena _ jsmartinez'\n");
   fprintf(fp,"set ytics(");
   for (it = head(procesos); it != 0; it = next(it)) {
       p = (process *)it->data;
      fprintf(fp,"'%s' %d%s",p->name,i , it->next !=0? ",":"" );
      i++;
   }
   fprintf(fp," ) \n");
